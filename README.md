...
| **Bio Banner Builder** | Design headers for GitHub, LinkedIn, X, YouTube, and portfolio pages with platform presets or custom dimensions, background styles or uploaded images, eyebrow/headline/subtitle/CTA layers, accent color, image position, text scale, overlay strength, alignment, avatar visibility, handle visibility, and safe-area controls. |
...
| **Export Controls** | Choose filename, 1×/2×/3× scale, transparent or white background, exact card/banner dimensions, and PNG/JPEG output for banners. |
...
The preview scales to the selected aspect ratio, while export renders the design at the exact preset or custom dimensions. You can add a local background image, choose its position, adjust a readability overlay, scale the text block, show or hide the safe-area guide, and export as PNG or JPEG. The export path uses [html2canvas](https://html2canvas.hertzen.com/) and includes compatibility-safe styling for the browser renderer.
...
To build a banner, open **Banner Builder** from the workspace sidebar, select a platform preset or **Custom canvas**, choose a background style or upload a local image, edit the eyebrow, headline, supporting line, CTA, accent, alignment, image position, text scale, overlay, and visibility controls, then choose PNG or JPEG and download the result. The banner automatically uses the current name, handle, identity line, and avatar from the profile workspace.
...
Manual browser verification should cover profile editing, quality warnings, template application, undo/redo, brand-kit application, project export/import, saved-project actions, banner preset changes, custom canvas sizing, local background-image upload/removal, text layers, image positioning, text scale, overlay strength, safe-area visibility, responsive preview behavior, offline shell registration, PNG/JPEG export, and exported image dimensions. AI generation requires a valid Groq key and network access.
